	...
	if [ $UNIFIED_HW ] || [ $EXAMPLE_DESIGN ]; then
		...
		if [[ $BOARD = "ti375c529" || $BOARD = "ti375n1156" ]]; then
			if [[ $MACHINE_ARCH = 64 ]]; then
				...
				#define SYSTEM_AXI_SLAVE_3_IO_CTRL 0xe8820000
				#define SYSTEM_AXI_SLAVE_3_IO_CTRL_SIZE 0x10000
				#define SYSTEM_AXI_SLAVE_4_IO_CTRL 0xe8830000
				...
				#define SYSTEM_AXI_D_BMB 0xe8820000
				#define SYSTEM_AXI_D_BMB_SIZE 0x10000
				#define SYSTEM_AXI_E_BMB 0xe8830000
...
function generate_device_tree() {
	...
	build_dt_cmd() {
		...
		if [[ "$type" == "linux" ]]; then
			add_cfg_if_exists cmd "$generic_dir/reserved_memory_kernel_6_6.json"
		fi
